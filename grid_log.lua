...
if grid then
    grid = grid.grid_api("grid") -- ask for grid api, you can replace uuid with your name
else
...
local function uuidToColor(uuid)
...
function events.render(delta)
    if grid:canEdit() then
        grid:setDepth(math.cos(world.getTime(delta) * 0.2))
    end
end